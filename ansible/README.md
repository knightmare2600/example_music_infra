...
│       ├── main.ini            — generated by ansibleme.sh (pvenodes, ansiblehosts, dcs, firewalls)
│       ├── rudder.ini          — Rudder server and relay hosts
│       ├── salt.ini            — Salt master host (Windows client-endpoint minions only)
│       ├── <site>.ini          — one per site (aar.ini, abd.ini, ... vrk.ini — 53 total),
│       │                         auto-generated by benarbejde/generate_inventory.py from
│       │                         sites.csv + devices.csv. DO NOT EDIT — see that file's own
│       │                         "DO NOT EDIT" header. windows_dc/windows_server/windows/
│       │                         windows_nodes group hierarchy included in every site file.
│       ├── group_vars/         — see "group_vars location" below — this is not cosmetic
│       │   ├── all/
│       │   │   ├── colours.yml     — ANSI colour constants (_c.R/G/Y/O/C/W/NC)
│       │   │   ├── main.yml        — vars for every host
│       │   │   ├── site_services.yml — generated, see benarbejde/generate_inventory.py --emit-group-vars
│       │   │   ├── vars.yml        — additional global vars
│       │   │   └── vault.yml       — encrypted secrets
│       │   ├── firewalls/main.yml  — WireGuard hub reference data (pubkeys, IPs, topology, extra subnets)
│       │   ├── pvenodes/main.yml
│       │   ├── rudder_relays/main.yml
│       │   ├── rudder_servers/main.yml + vault.yml
│       │   ├── salt_servers/main.yml
│       │   ├── windows/vars.yml
│       │   ├── windows_dc/vars.yml
...
│       └── host_vars/
│           ├── EXARUDCLD001/main.yml
│           ├── EXASLTCLD001/main.yml
...
### `group_vars`/`host_vars` location — read before moving these

`group_vars/` and `host_vars/` live under `configs/inventory/`, not at the
`ansible/` root where older Ansible repos usually put them. This is
load-bearing, confirmed empirically, not a style choice: Ansible's
`ansible.builtin.host_group_vars` vars plugin only auto-loads `group_vars/`/
`host_vars/` from *inside* the actual loaded inventory path (its own docs:
"only applies to inventory sources that are existing paths") — a sibling
directory does not work, only nesting inside does. `group_vars/` used to sit
directly at `ansible/`, which is *not* inside `ansible/configs/inventory/`
(the actual `inventory =` target in `ansible.cfg`), so it silently never
applied to any numbered playbook, regardless of correct inventory group
membership — this was the root cause of the `registry_common`/
`domain_ou_role` undefined-variable crashes found in the 2026-07-09
idempotency audit. A symlink-based fix was tried first and reverted: this
repo is cloned on Windows, Linux, and macOS, and symlinks don't survive that
reliably. Moving the real directory is the only fix that's identical on
every platform. If `ansible.cfg`'s `inventory =` path ever changes, `group_vars`/
`host_vars` need to move with it.
...
This used to be split across two locations — `configs/inventory/` (hand-curated,
DC-onboarding-only, covering 3 sites) and a separate `inventory/` (this repo's
generator's own output, covering fewer sites still, with a different and
incompatible group structure — no `windows_dc`/`windows_nodes` at all). Whichever
one happened to be loaded silently determined whether DC-specific `group_vars`
applied. Consolidated 2026-07-09: `generate_inventory.py` now writes directly into
`configs/inventory/`, one `.ini` per site, with the full `windows_dc → windows_server
→ windows → windows_nodes` hierarchy every group_vars/windows*/ file expects. The
old `inventory/` directory is retired.

| File | Contents |
|------|----------|
| `main.ini` | Generated by `ansibleme.sh` — pvenodes, ansiblehosts, dcs, firewalls from discovery |
| `rudder.ini` | Rudder server and relay hosts |
| `salt.ini` | Salt master host (Windows client-endpoint minions only) |
| `<site>.ini` | One per site (53 total) — generated by `benarbejde/generate_inventory.py`. DO NOT EDIT; re-run the generator after changing `sites.csv`/`devices.csv` |

Regenerate all site files:
```bash
python3 benarbejde/generate_inventory.py benarbejde/sites.csv -o ansible/configs/inventory --devices benarbejde/devices.csv
```
...
**host_vars/EXARUDCLD001/main.yml**

| Variable | Example | Description |
|----------|---------|-------------|
| `rudder_hostname` | `EXARUDCLD001` | Hostname (uppercase) |
| `rudder_static_ip` | `192.168.69.12` | Static IP |
| `rudder_gateway` | `192.168.69.253` | Default gateway |
| `rudder_dns` | `192.168.139.8` | DNS resolver (`EXADNSVRK001`, BIND9 — *not* the AD DC, see `rudder_dc_hostname`/`rudder_dc_ip` above) |
...
Bootstraps the Salt master (`EXASLTCLD001`) that manages every Windows node — client
endpoints, member servers, and domain controllers (`WKS`/`LAP`/`SUR`/`SVR`/`DCS` per
`benarbejde/role_codes.csv`'s `SLT` row). Not `FWL`/`PVE`/Linux generally. `TAB` only
counts when `devices.csv`'s `OS` column genuinely says Windows (most real `TAB` rows are
Android/iPadOS). `MAC`/`MBP` (macOS) are future plans, not current scope. Scope revised
twice on 2026-07-20: first widened from "client endpoints only" to include `SVR`, then
`DCS` was added too, on Robert's explicit call. This scope explicitly supersedes the old
...
(every Windows role, via `generate_inventory.py`'s own `windows_dc → windows_server →
windows → windows_nodes` chain) over SSH, near the end of the normal windows_bootstrap
chain — after domain join, well after the host's real hostname is set. Deliberately
**not** installed via the unattend XML at Setup time — that was the first approach
(same day), reverted once it became clear a random Setup-time hostname would pollute
the master's key list with dead entries every build. Points minions at
`salt.jukebox.internal` (a CNAME, `benarbejde/role_codes.csv`'s new `DNSAlias` column) rather
than a hardcoded IP.
See `docs/buildsheets/buildsheet-salt-minion.md` for the full mechanism and the manual
fallback.

### Quick start

**Step 1 — Inventory is already in `configs/inventory/salt.ini`.**

**Step 2 — host_vars are pre-filled**

`host_vars/EXASLTCLD001/main.yml` is pre-filled for the CLD master. For additional
masters (not expected at current scale — see Notes below) create
...
**Step 4 — Accept minion keys as endpoints check in**

```bash
# On EXASLTCLD001
salt-key -L                    # list pending/accepted keys
salt-key -a <minion-name>      # accept a specific minion
...
| Variable | Default | Description |
|----------|---------|-------------|
| `salt_domain` | `jukebox.internal` | Used for `/etc/hosts`/MOTD only — Salt itself doesn't need AD/LDAP |
...
| Variable | Example | Description |
|----------|---------|-------------|
| `salt_hostname` | `EXASLTCLD001` | Hostname (uppercase) |
| `salt_static_ip` | `192.168.69.22` | Static IP |
| `salt_gateway` | `192.168.69.253` | Default gateway |
| `salt_dns` | `192.168.139.8` | DNS resolver (`EXADNSVRK001`, BIND9) |
| `salt_prefix` | `24` | Subnet prefix length |
| `salt_network_interface` | `""` | Interface (blank = auto-detect) |
| `salt_site_code` | `CLD` | Site code (matches sites.csv) |

### Notes

**No Syndic tier at current scale**
Salt's hierarchical master-of-masters scaling mechanism (analogous to what a Zabbix
proxy does) is very likely premature for ~49 client-endpoint devices. If ever needed,
it would go at a handful of regional hub sites, not as a universal per-site standard
slot — `.22` is *not* a per-site convention, unlike `NAS`'s `.19` or `RDR`'s `.21`.

**Version alignment is deliberate, not incidental**
