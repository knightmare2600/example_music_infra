...
`.19` IP, SSH enabled, at least one pool created) before any of this runs.
...
mechanism `firewalls`/`windows_dc`/etc already use. **No real row exists
yet** — this group has zero members in every currently-generated `.ini`.

## Open, unresolved

- **CORE vs SCALE** not yet decided (per `proxmox-dcm-pbs-planning.md`) —
  the collection's modules talk to the same middleware API either way, so
  nothing here hardcodes one, but it's untested against either.
...
- **Nothing here has been run against a real TrueNAS box.** Parameter names
  are confirmed against the installed collection's real argspec
  (`ansible-doc -j arensb.truenas.*`), not just its rendered docs — but
  live behaviour against a real middleware daemon is unverified.
