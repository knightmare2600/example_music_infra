...
| `begyndelse.json` | Well-known service addresses (provisioning servers, DNS, Ansible control node, PBX, Rudder, WAC) derived from the two CSVs above — generated by `generate_inventory.py --emit-begyndelse-json`. Same data as `ansible/group_vars/all/site_services.yml`, in JSON, for non-Ansible consumers (`bindme.sh`, `ansibleme.sh`, `firewallme.sh`) to `jq` instead of hardcoding IPs |
