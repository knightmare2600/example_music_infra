...
def emit_group_vars(csv_path: Path, devices_path: Path) -> str:
  ...
  hostnames/IPs derived from sites.csv + devices.csv, plus the concrete exa_asset_base/
  exa_wallpaper_url variables every windows_bootstrap play already reads today.
  """
  rows = list(csv.DictReader(csv_path.open()))
  validate_csv_structure(rows)
...
#
# Replaces hardcoded literals previously scattered across ansible/group_vars/all/vars.yml,
# windows_bootstrap/playbooks/05-bootstrap.yml, and others. If a well-known service's IP ever
# changes, fix its devices.csv row and regenerate — not every playbook that references it.
# =============================================================================
...
# Convenience variables — the actual names windows_bootstrap playbooks already read today.
# Edinburgh is the default/primary provisioning server; Fredericia Havn is the standby.
exa_asset_base: "{prv_edi_url or ''}/WINDOWS"
exa_wallpaper_url: "{prv_edi_url or ''}/ExampleMusicWallpaper.png"
"""
