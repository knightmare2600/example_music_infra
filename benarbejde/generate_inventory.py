...
def emit_site_grains_pillar(csv_path: Path) -> str:
  """
  Returns the full text of salt/pillar/sites.sls -- a Site -> {city, country, entity, street,
  postal_code} lookup, read directly from sites.csv's own columns. No hand-duplication: Entity
  ...
  street/postal_code custom grains from a minion's own site code, the same way every other role
  in this estate (Rudder, Salt master itself, etc.) already gets site_city/site_country/
  site_entity -- via a real sites.csv lookup, not baked into the hostname.

  street/postal_code (2026-08-04, Robert): where the site's network infrastructure physically
  sits, not a general office/employee-workplace address -- see benarbejde/sites.csv's own
  Street/PostalCode columns. Empty string where sites.csv itself has the column blank (a few
  sites still have no postcode on record), same "blank is fine, don't fabricate" treatment as
  city/country/entity already get.
  """
  rows = list(csv.DictReader(csv_path.open()))
  validate_csv_structure(rows)
  ...
  sites = {
    r["Site"]: {
      "city": r["City"], "country": r["Country"], "entity": r["Entity"],
      "street": r.get("Street", ""), "postal_code": r.get("PostalCode", ""),
    }
    for r in rows
  }
