...
# ansible/group_vars/all/vars.yml's exa_asset_base, windows_bootstrap/playbooks/05-bootstrap.yml's
...
def emit_group_vars(csv_path: Path, devices_path: Path, ad_forest_path: Path = None) -> str:
  ...
  return f"""\
# =============================================================================
# ansible/group_vars/all/site_services.yml
...
def main():
  ...
  parser.add_argument(
    ...
    help="Write ansible/group_vars/all/site_services.yml — well-known service addresses "
         "(provisioning servers, DNS, Ansible control node, PBX, Rudder, WAC) derived from "
         "sites.csv + devices.csv, instead of writing .ini files."
  )
  parser.add_argument(
  ...
  try:
    ...
    if args.emit_devices_json:
      ...
      group_vars_out = args.group_vars_out or (
        Path(__file__).resolve().parent.parent / "ansible" / "group_vars" / "all" / "site_services.yml"
      )
