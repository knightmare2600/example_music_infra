...
- [x] Phase 2 — `benarbejde/generate_network_diagrams.py`: New Network box
      generated from `sites.csv`/`devices.csv`/`address_policy.csv` for all
...
| 6 | Generated-file freshness | `check_generated_freshness.py` — regenerates `configs/inventory/*.ini`, `site_services.yml`, `begyndelse.json`, and `salt/pillar/sites.sls` (added 2026-07-21) from `benarbejde/sites.csv`+`devices.csv`+`address_policy.csv`+`ad_forest.json` into a scratch dir and diffs against committed — catches "edited the source, forgot to regenerate" |
...
| 20 | Role code registry | `check_role_codes.py` — `benarbejde/role_codes.csv` (every device-role code's name/category/connection-method/emoji — added 2026-07-20 consolidating three separate hand-maintained copies, one of which had already drifted: MBP was `ssh` in one, `winrm` in another) checked against `docs/emojis/README.md`'s hand-maintained legend, which must agree with it exactly |
| 21 | Salt state validity | `check_salt_states.py` — Salt's own equivalent of check 2, plus two Salt-specific structural checks with no Ansible analogue. Every `salt/states/**/*.sls` (bar `_modules/`) renders via Jinja2 (a generic mock `grains`/`pillar`/`salt` context covering every real `salt['module.function'](...)` call found in these files) and the result parses as YAML — nothing previously checked a broken Jinja expression or an undefined grain/pillar lookup in Salt content at all, the same gap `--syntax-check` fills for Ansible. Also: every state/pillar name in `top.sls`/`pillar/top.sls` resolves to a real `<name>.sls` or `<name>/init.sls`; every `messagetype:` value used in a `screenprint.screen_print` call is one `screenprint.py`'s own `MESSAGETYPE_COLOR` dict actually maps to a colour (read from the module directly, not hand-copied here); every `salt/states/` module directory is named in `salt/README.md`. Added 2026-07-21 after `messagetype: "warn"` (not a real value) sat silently uncaught in a ported file, found by eye rather than by anything in this harness |
| 22 | Duplicate device hostnames | `check_duplicate_devices.py` — every generated `configs/inventory/<site>.ini` and `generate_inventory.py --emit-devices-json`'s merged output, for the same hostname appearing twice at two different IPs (a standard-slot template line and a real `devices.csv` row colliding on the same role+instance number). Added 2026-07-22 after this exact bug turned up live twice — `EXAFWLVRK001` in `vrk.ini` and `EXAWKSFAL001`/`EXALAPFAL001` in `fal.ini`, both found only by eye. `bind9-dns.yml`'s zone templates consume `--emit-devices-json` via a Jinja `first` filter, which would silently pick one IP and drop the other rather than erroring — this check is the only thing that catches that class of bug before it reaches a live zone file |
| 23 | Network session safety | `check_network_session_safety.py` — every playbook/task file for the `nmcli` delete+recreate-same-connection antipattern (hard fail) and an ungated `nmcli connection up <profile>` on a profile the same file templates (informational unless `--strict`). Added 2026-07-22 after the exact bug — unconditional `nmcli con delete X` + `nmcli con add ... con-name X` on every run, always reporting "changed" — turned up live in `bind9-dns.yml` (killed Robert's SSH session on `EXADNSVRK001`), then the identical copy-pasted pattern in `rudder_server.yml` and `salt/playbooks/10-master.yml` |
| 24 | Control node's own data freshness | `check_control_node_freshness.py` — if `/etc/example-music/` exists on the machine running this harness (skips cleanly otherwise — not the control node), hash-compares every file `linux/tools.yml` deploys there (`sites.csv`, `devices.csv`, `role_codes.csv`, `address_policy.csv`, `ad_forest.json`, `ad_groups.json`, `ad_users.json`, `ad_computers.json`) against its `benarbejde/` source. Added 2026-07-27 after `bind9-dns.yml` crashed live with a raw Python `KeyError` — traced to the control node's own served `address_policy.json` predating a `benarbejde/` schema change, because `linux/tools.yml` had never been re-run against the control node itself. This is the offline half of that fix; the in-playbook half is `ansible/tasks/example_music_freshness_gate.yml` |
| 25 | Bootstrap assets | `check_bootstrap_assets.py` — two tiers. Tier 1 (always hard fail): `bootstrap/web/proxmox/*.toml` answer files, derived from `select-pve-answer.sh`'s own `site_prefix`/`variant` literals rather than hand-copied a third time. Tier 2 (informational unless `--strict`): kernel/initrd/iPXE-fetched boot binaries referenced by `bootstrap/web/menu.ipxe`, with the small set of enumerable iPXE variables it uses expanded programmatically. Added 2026-07-27 |
| 26 | Collection requirements | `check_collection_requirements.py` — two hard-fail checks. (1) Every collection FQCN module actually used in a playbook module directory must be declared in that directory's own `requirements.yml`, detected by walking real YAML task nodes (not a blind text regex — avoids false positives like `item.item.fqdn` or `example1.co.uk` in a URL). (2) Every module directory with a `requirements.yml` must document the install command somewhere a human would see it (its own README, or the playbook's own header comment). Added 2026-07-28 after a real `windows_bootstrap/site.yml` run against `EXADCSCLD001` completed, then the very next module (`truenas/site.yml`) failed outright with `"No module named 'ansible_collections.arensb'"` — the collection was only ever installed on the sandbox that built the module. Also found live the same day: `windows_dc/` genuinely uses `ansible.windows.*` modules (real DC promotion) with no `requirements.yml` at all and no README mention of installing anything |
| 27 | SubnetSite mismatches | `check_subnet_site_mismatch.py` — informational only, never fails. Flags every `devices.csv` row where `SubnetSite` is set and differs from `Site` (a device hostnamed under one site but physically on another's subnet — e.g. `EXAPBXCLD002`, hostnamed under CLD, real IP on FRD Havn's `172.16.124.0/24`). A legitimate, deliberate pattern, not a bug — the point is visibility, since nothing previously surfaced this anywhere outside the CSV's own comment column. Added 2026-07-30 after Robert had to find the FRD/CLD PBX case by asking directly rather than it showing up anywhere |
| 28 | Doc role coverage | `check_doc_role_coverage.py` — cross-references `generate_inventory.py`'s real per-site device list against `docs/site-inventory.md`'s per-site sections; flags any real, addressed device whose hostname doesn't appear anywhere in its own site's section. Informational (Tier 2, `--strict` fails on it) — this is a real, large, pre-existing documentation gap (284 findings across 42 sites as of 2026-07-30, mostly predating the 2026-07-26 NAS/RDR/BMC/WAP synthesis policy), not something to block every unrelated commit on until it's manually reconciled. Added 2026-07-30 after the CLD NAS/RDR/BMC gap was found only by a manual read, per Robert's own "add a check for new 'things'" ask |
| 29 | DCR devices | `check_dcr_devices.py` — informational only, never fails. Flags every `devices.csv` row with `Type=DCR` ("Domain controller (legacy naming)" in `role_codes.csv`) — `generate_network_diagrams.py`'s `TOPOLOGY_EXCLUDE_TYPES` hides these from every topology diagram (found leaking onto EDI's/TOR's "new network" diagrams 2026-07-31 — genuinely legacy/decommission-pending hardware, e.g. `EXADCRTOR028`'s "Undocumented legacy AD install... no-one on record knew it existed"), which would otherwise make them invisible everywhere, not just fixed. Added 2026-07-31, Robert: "add a check to the harness to catch DCR and alert if found" |
| 30 | Legacy device hostname collisions | `check_legacy_devices.py` — `benarbejde/legacy-devices.csv` (old-network-only core infra with no live counterpart: RAC/iLO/iDRAC, ESXi hosts, vCenter) for structural validity, unknown Site codes, duplicate rows, Types outside RAC/ESX/VCT/RTR, and — the main point — any row whose computed hostname collides with a real hostname in the live generated inventory, except a small, explicitly-confirmed `ALLOWED_HOSTNAME_REUSE` set (a decommissioned device's real historical hostname genuinely reused by its physical replacement, e.g. `EXARTRFAL001`'s old Cisco → the FortiGate that replaced it). Added 2026-08-03 directly because of the `EXAFWLFAL001` mess: a hand-built Old Network diagram invented a device under a hostname that turned out to already belong to a real, live, current device. Extended 2026-09-24 with 11 more confirmed-genuine reuse entries (`SYD`/`MEL`/`AKL`/`BIR`/`BON`/`CPH`/`EDI`/`LAX`/`LIV`/`MCR`/`NEW`'s own decommissioned-ESXi-host BMCs, reused by their real replacement hardware) found while filling in the estate's missing-`devices.csv`-row backlog |
| 31 | Topology diagram freshness | `check_topology_diagram_freshness.py` — `render_topology_block()`'s "Topology sketch" mermaid block (`insert_topology_into_docs()`), the sibling generator to check 14's flat "New Network" box — same regenerate-into-scratch-and-diff technique. Added 2026-08-04 after check 14's own blind spot let 22 sites' *current* topology sketches silently drift and show the stale, pre-rename `EXADCR<SITE>001` hostname mislabeled as "DCS 1" — caught only by hand while regenerating for an unrelated switch-vendor edit |
| 32 | Old Network diagram freshness | `check_old_network_freshness.py` — `render_old_network_block()`'s "Old Network" mermaid block, the sibling generator to check 31. Added 2026-08-04: the generator function had existed, fully built, since before the 2026-07-31 restyling, but was never actually called from `main()` — the `GENERATED:OLDNETWORK` marker already in every site's doc implied a freshness guarantee that, per check 31's own predecessor gap, didn't actually exist. Real drift had already accumulated by the time this was noticed (a workstation `devices.csv` addition and an edited router note, both missing from FAL's frozen Old Network box) |
| 33 | Leaked secrets (gitleaks) | `check_gitleaks.py` — scans the full git history via [gitleaks](https://github.com/gitleaks/gitleaks). Robert's ask, 2026-08-04: hook gitleaks into the harness itself, not just a pre-push hook — found live there wasn't a pre-push hook either, `docs/gitleaks_guide.md` was a manual how-to reference that nothing actually ran. Host-local-tool pattern (informational if gitleaks isn't installed, `--strict` fails on that) — but a genuine finding is **always** a hard failure regardless of `--strict`; there's no "soft" leaked credential. Uses `.gitleaks.toml`'s allowlist |
| 34 | Role code usage | `check_role_code_usage.py` — `devices.csv`'s own `Type` column cross-referenced against `role_codes.csv`'s `Code` column (the direction check 20 does *not* cover — that one only cross-checks `role_codes.csv` against the emoji legend). Hard fail on any unknown `Type`; `--strict` additionally flags `role_codes.csv` codes defined but never used by any real row (informational — a code can legitimately exist ahead of any device using it, e.g. `RRY`). Added 2026-08-05/06 after `create-vm.py` hit "Unknown role code" against a live PVE node — turned out to be a stale deployed `role_codes.csv` on that one host, not a repo bug, but it exposed that nothing checked this pairing at all |
| 35 | Data-refresh doc coverage | `check_data_refresh_doc_coverage.py` — `docs/refresh-after-data-changes.md` names every playbook that includes `ansible/tasks/example_music_freshness_gate.yml`, plus `linux/tools.yml` itself (the only playbook that actually deploys `benarbejde/*` to `/etc/example-music/*`). Hard fail if a future gated playbook, or `linux/tools.yml`, stops being named there. Cannot and does not check any live host's actual on-disk staleness — see the doc's own final section for why that's structurally out of scope. Added 2026-08-06, Robert: "it needs a small doc and harness checks to boot, where it says 'the following playbooks need run on the following hosts'" |
| 36 | Required host-local tools | `check_required_tools.py` — generalises the same host-local-tool pattern checks 11/13/33 each hand-roll for one tool, across every tool a control-node script actually needs (informational unless `--strict`), scoped deliberately to control-node tools only (not target-host tools, which already self-heal via their own `command -v X \|\| BOOTSTRAP_PKGS+=(x)` pattern). Added 2026-08-06, Robert: "the harness is supposed to keep track of what packages are needed and check if they are installed" after `kpcli_wrapper.py` failed outright (missing `keepassxc-cli`) with no prior warning. Extended 2026-08-08 with a `python_module` check type (via `importlib.util.find_spec` — a PATH binary check can never find a Python library) after `passlib` turned out missing on the real `EXAANSCLD001` |
| 37 | Workstation tool version pins | `check_workstation_tool_versions.py` — `benarbejde/asset_manifest.json`'s `workstation_tools[]` pinned tag checked against the real upstream GitHub release: every listed per-platform asset still exists under the pinned tag (hard fail if not — a broken pin), and whether the pin is behind the real "latest" release (informational, `--strict` promotes). Robert's ask, 2026-08-08: "have the harness check for new versions, verify checksums, all the usual spangly stuff" |
| 38 | Windows architecture fact usage | `check_windows_arch_fact.py` — flags any bare `ansible_architecture` usage under a `windows_*` playbook directory. Real bug, found live 2026-08-11: on Windows this fact reports OS *bitness* ('64-bit'/'32-bit'), not CPU architecture — it can't distinguish AMD64 from ARM64 at all. A shared task file silently mismapped `host_arch` to `'x86'` on every real Windows box since 2026-06-20, never caught until the first real Salt-minion run against `EXADCSCLD001`. Fixed to use `ansible_architecture2`. Directory-scoped, not a blanket ban — `ansible_architecture` is genuinely correct on Linux and is deliberately not flagged there |
| 39 | Hardcoded site metadata | `check_hardcoded_site_metadata.py` — flags any `host_vars/*/*.yml` file defining a hand-typed `<prefix>_site_(city\|country\|country_code\|entity\|office_name\|street\|postal_code)` key. Real bug, found live 2026-08-13: Salt/Rudder/TacticalRMM all hardcoded their site's Country as `"Scotland"` instead of deriving it live from `sites.csv` — wrong for CLD (`"Global"`, a virtual/cloud site) — despite one of the three's own comment claiming it had been checked against `sites.csv`. Robert: "everything is supposed to use sites.csv... hard-coding stuff is very 1970s and verboten here" |
| 40 | Windows guest-tools staging | `check_windows_guest_tools_staging.py` — confirms `Deploy-OpenSSH.cmd` stages the required amd64 fallback KVM/Proxmox guest driver files (`virtio-win-gt-x64.msi`, `qemu-ga-x86_64.msi`) for both x86_64 and arm64, and that any vendored native arm64 MSIs are real git-lfs-resolved content, not unresolved pointer stubs |
| 41 | Break-glass CSV field alignment | `check_breakglass_csv_fields.py` — two checks against the break-glass bash scripts (`firewallme.sh`/`bindme.sh`/`rudderme.sh`/`ansibleme.sh`), each of which parses `sites.csv` with a fixed, positional `while IFS=',' read -r <names...>` line rather than reading by column name. (1) **Field alignment**: every named field still sits at the same position as the matching column in `sites.csv`'s real current header — real live failure this prevents recurring: `EXAFWLCOV001`, 2026-08-29, `sites.csv` gained four columns and every one of these four scripts' read lines went stale at the same time, unnoticed. Robert: "we need harness checks because this is an operational gap." (2) **Naive-split safety**, independent of any script: every `sites.csv` row naive-comma-splits into the same field count as a real quote-aware CSV parse — a row that doesn't (a properly-quoted comma inside a Street address) corrupts every break-glass script's read of it regardless of field-list correctness. Found live the same day (BRT, MIL); BRT is a documented, deliberate exception (`KNOWN_NAIVE_SPLIT_EXCEPTIONS`) so this still catches any *future* case without permanently failing on the one already-accepted one |
| 42 | Break-glass reverse-zone collisions | `check_breakglass_zone_file_collisions.py` — found live 2026-09-21 building `EXADNSFRD001`: `bindme.sh`'s per-site reverse-zone generation writes to a path derived from each site's own subnet prefix — VRK's subnet is numerically identical to the provisioning network's own already-generated zone file, but the per-site loop only ever excluded CLD, never VRK, silently truncating and overwriting the real provisioning zone on every build (including every prior build of `EXADNSVRK001` itself — `named-checkzone`/`rndc zonestatus` both reported the zone loading cleanly; only `dig -x` against a real PTR ever surfaced it). Cross-references `sites.csv`'s real subnet data against `bindme.sh`'s own per-site loops structurally (via their file-path construction, not comment wording) and fails if any subnet-sharing site isn't excluded from every one of them |
| 43 | AD data integrity | `check_ad_data_integrity.py` — the largest single check here, 13 independent sub-checks (A–M) against `ad_users.json`/`ad_groups.json`/`ad_computers.json`, none needing a real AD connection. Added 2026-09-23 after one evening's `populate_ad` debugging surfaced four distinct data bugs, each only ever caught by a real `New-ADUser`/`New-ADComputer` failure: a `SamAccountName` collision between a group and a user (`"The specified group already exists"`), an `ad_users.json` `Groups:` reference resolving by the wrong field, a straight `SamAccountName` typo, and a computer's `ad_ou` missing its site's Province level (`"Directory object not found"`). Grew substantially over 2026-09-24: `IPv4Address` drift against `devices.csv`'s own real value for a hostname; same-site duplicate IPs with **failsafe triangulation** (a third signal, `address_policy.csv`'s own convention, resolves which of two disagreeing records is actually wrong — Robert's own naming: "if all three disagree we print a message and the user has to decide, this is called failsafe"); duplicate `DNSHostName` values; a real record's Type having no `devices.csv` row at all despite a real addressing convention existing for it; a claimed octet landing exactly on a *different* Type's reserved slot (caught `EXARACBIR001`'s `.6` as PVE's own address, from Robert live: "is that a BMC address? No, .6 is a PVE host IP"); two real devices sharing one live IP; every standard site missing a switch or router entirely; and malformed IPv4 octets (`192.168.361.x` — a stray digit, found while transcribing a devices.csv row). A non-lettered PVE-count-inference advisory (real ILO/RAC records imply a real PVE node behind each one, since ad_computers.json never tracked PVE directly) was RETIRED 2026-09-26 — `benarbejde/merge_ad_computers.py` now gives every real `devices.csv` PVE node its own direct `ad_computers.json` record, so the inference is superseded, not just redundant |
| 44 | New-site boilerplate | `check_new_site_boilerplate.py` — Robert, 2026-09-24, on `PHI`/`DET` (real `sites.csv` entries with zero equipment anywhere): "the harness needs to look for and identify new sites which do not have any devices... you know the IPs, the subnets, the other stuff." Flags any site with *literally* zero rows in `devices.csv` and zero records in `ad_computers.json` (a much broader signal than check 43's per-device-type gaps) and prints the full confirmed day-one boilerplate — real, computed hostnames and IPs for the router/firewall/switch/WAP/SBC/badge-reader/NAS/two-Proxmox-nodes-plus-BMCs/domain-controller set every standard site gets (see `benarbejde/standard_site_boilerplate.json`). Advisory only, never fails — but the harness Summary prints a standing nudge with the exact fix command whenever the count is non-zero. Gained two write modes the same evening, both an explicit opt-in the read-only default never triggers: `--apply <SITE>` (a genuinely empty site — writes the full boilerplate as `Planned=yes`) and `--complete <SITE>` (a partially-built site — fills in only the Types it's actually missing, e.g. `FAX` already had a router/firewall/switch/WAPs but no badge reader, NAS, or Proxmox nodes at all) |
| 45 | Jinja test-name validity | `check_jinja_test_names.py` — found live 2026-09-21: `select('length')` in `30-ad-users.yml` — `'length'` is a real Jinja token, just the wrong *kind* (a filter name, not a test name), so `select()` only fails at render time (`"No test named 'length'"`), not at parse/compile time. Neither `ansible-lint` (even `-p production`) nor `ansible-playbook --syntax-check` catches this — both validate syntax/formatting only, never attempt to actually render. Greps every `select()`/`reject()`/`selectattr()`/`rejectattr()` call for a quoted test-name argument and flags any name that isn't real, checked against `ansible-doc -t test -l`'s **live output** (not a hardcoded snapshot that could drift), scoped to exactly the collections this repo's own `requirements.yml` files declare. Added 2026-09-24 once Robert was back in the UK (deferred from 2026-09-21) |
| 46 | Vault placeholder values | `check_vault_placeholders.py` — found live 2026-09-25: `populate_ad`'s `New-ADUser` calls against `EXADCSGOT001` failed AD's password complexity check because `vault_ad_new_user_password` (`group_vars/windows_dc/vault.yml`) was still the literal placeholder `"CHANGEME"`, never actually set — it had evidently worked once before against the now-decommissioned `EXADCSFRD001` test forest (likely a relaxed password policy there), so the unfilled placeholder itself was never caught until a genuinely fresh forest's standard policy rejected it. Sweeping further found 4 more `CHANGEME` values (`group_vars/all/vault.yml`) and a different placeholder convention entirely (`REPLACE_WITH_...`/`UNSET`) in `truenas_servers`/`rudder_servers` vault files that hadn't been hit live yet either. Also confirmed via `ansible-vault view` and git history that none of this repo's `vault.yml` files have ever actually been ansible-vault encrypted — parses them as plain YAML (skipping, not failing, any that genuinely are encrypted) and flags any value matching a known placeholder convention |
| 47 | nodeinfo.json coverage | `check_nodeinfo_coverage.py` — Robert, 2026-09-26: "I want every playbook to update that JSON field" (nodeinfo.json's `last_ansible_run`). `windows_bootstrap`/`windows_dc`/`windows_adschema` never wrote `nodeinfo.json` at all until this same day, discovered only because Robert asked directly whether every playbook did. Fails if any playbook family that manages a real host doesn't reference `ansible/tasks/nodeinfo.yml` or the new `nodeinfo_windows.yml` anywhere in its own file tree — `meshcentral` (dead/retired), `snmp` (targets network devices over SNMP, not hosts with a filesystem), and `ssh_preflight_with_fallback.yml` (a pre-connectivity utility that runs before any real work happens) are the only exemptions, and they're explicit in the check's own code, not silently overlooked |
