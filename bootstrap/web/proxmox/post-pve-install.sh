  ...
  if find /etc/apt/sources.list.d/ -maxdepth 1 -name '*.sources' | grep -q .; then
    ...
    check_and_disable_legacy_sources() {
      ...
      if ((LEGACY_COUNT > 0)); then
        ...
        whiptail --backtitle "Proxmox VE Helper Scripts" --title "Disable legacy sources?" --yesno "$MSG" 18 80
        if [[ $? -eq 0 ]]; then
...
EOF
  ...
  if component_exists_in_sources "pve-enterprise"; then
    CHOICE=$(whiptail --backtitle "Proxmox VE Helper Scripts" --title "PVE-ENTERPRISE" --menu "'pve-enterprise' repository already exists.\n\nWhat do you want to do?" 14 58 2 "keep" "Keep as is" "disable" "Comment out (disable) this repo" "delete" "Delete this repo file" 3>&2 2>&1 1>&3)
    case $CHOICE in
    keep)
    ...
    CHOICE=$(whiptail --backtitle "Proxmox VE Helper Scripts" --title "PVE-ENTERPRISE" --menu "The 'pve-enterprise' repository is only available to users who have purchased a Proxmox VE subscription.\n\nAdd 'pve-enterprise' repository (deb822)?" 14 58 2 "no" " " "yes" " " --default-item "no" 3>&2 2>&1 1>&3)
    case $CHOICE in
    yes)
...
EOF
  ...
  if grep -q "enterprise.proxmox.com.*ceph" /etc/apt/sources.list.d/*.sources 2>/dev/null; then
    CHOICE=$(whiptail --backtitle "Proxmox VE Helper Scripts" --title "CEPH-ENTERPRISE" --menu "'ceph enterprise' repository already exists.\n\nWhat do you want to do?" 14 58 2 "keep" "Keep as is" "disable" "Comment out (disable) this repo" "delete" "Delete this repo file" 3>&2 2>&1 1>&3)
    case $CHOICE in
    keep)
  ...
  if [[ "$REPO_ACTIVE" -eq 1 ]]; then
    CHOICE=$(whiptail --backtitle "Proxmox VE Helper Scripts" --title "PVE-NO-SUBSCRIPTION" --menu "'pve-no-subscription' repository is currently ENABLED.\n\nWhat do you want to do?" 14 58 3 "keep" "Keep as is" "disable" "Comment out (disable)" "delete" "Delete repo file" 3>&2 2>&1 1>&3)
    case $CHOICE in
    keep)
    ...
    CHOICE=$(whiptail --backtitle "Proxmox VE Helper Scripts" --title "PVE-NO-SUBSCRIPTION" --menu "'pve-no-subscription' repository is currently DISABLED (commented out).\n\nWhat do you want to do?" 14 58 3 "enable" "Uncomment (enable)" "keep" "Keep disabled" "delete" "Delete repo file" 3>&2 2>&1 1>&3)
    case $CHOICE in
    enable)
    ...
    CHOICE=$(whiptail --backtitle "Proxmox VE Helper Scripts" --title "PVE-NO-SUBSCRIPTION" --menu "The 'pve-no-subscription' repository provides access to all of the open-source components of Proxmox VE.\n\nAdd 'pve-no-subscription' repository (deb822)?" 14 58 2 "yes" " " "no" " " 3>&2 2>&1 1>&3)
    case $CHOICE in
    yes)
...
post_routines_common() {
  CHOICE=$(whiptail --backtitle "Proxmox VE Helper Scripts" --title "SUBSCRIPTION NAG" --menu "This will disable the nag message reminding you to purchase a subscription every time you log in to the web interface.\n \nDisable subscription nag?" 14 58 2 "yes" " " "no" " " 3>&2 2>&1 1>&3)
  case $CHOICE in
  yes)
...
EOF
  ...
  if ! systemctl is-active --quiet pve-ha-lrm; then
    CHOICE=$(whiptail --backtitle "Proxmox VE Helper Scripts" --title "HIGH AVAILABILITY" --menu "Enable high availability?" 10 58 2 "yes" " " "no" " " 3>&2 2>&1 1>&3)
    case $CHOICE in
    yes)
  ...
  if systemctl is-active --quiet pve-ha-lrm; then
    ...
    yes)
      ...
      CHOICE=$(whiptail --backtitle "Proxmox VE Helper Scripts" --title "COROSYNC" --menu "Disable Corosync for a Proxmox VE Cluster?" 10 58 2 "yes" " " "no" " " 3>&2 2>&1 1>&3)
      case $CHOICE in
      yes)
