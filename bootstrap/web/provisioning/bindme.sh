...
# Static IP    : 192.168.139.10/24  (provisioning network, CLD .10 per SUFFIX_MAP)
...
#   192.168.139.10  ${THIS_HOSTNAME}  (this DNS server)
#   192.168.139.50  EXAPRVCLD001  (provisioning / PXE)
#   192.168.139.69  EXAANSCLD001  (Ansible management)
# ===============================================================
...
read -rp "Provisioning Network gateway [${PROV_NET}.1]: " GW_OCTET
GW_OCTET="${GW_OCTET:-1}"
...
options {
  ...
  // ── Listen ────────────────────────────────────────────────
  listen-on     { 127.0.0.1; 192.168.139.10; };
  listen-on-v6  { none; };
...
;
; Authoritative DNS: ${THIS_HOSTNAME} (192.168.139.10)
; Zone:              jukebox.internal
...
; -- DNS server itself -----------------------------------------
${THIS_HOSTNAME_LOWER}  IN  A  192.168.139.10

; -- CLD ancillary / provisioning infrastructure ---------------
; These are stable, hardcoded hosts in the provisioning subnet.
; They do not appear in the per-site generated block below.
exaprvcld001  IN  A  192.168.139.50   ; Provisioning server (PXE, HTTP, iPXE)
exaanscld001  IN  A  192.168.139.69   ; Ansible management node
...
; Server:  ${THIS_HOSTNAME} (192.168.139.10)
; Generated by: bindme.sh on $(date -u +%Y-%m-%dT%H:%M:%SZ)
;
; PTR records:
...
if dig @127.0.0.1 +short ${THIS_HOSTNAME_LOWER}.jukebox.internal | grep -q "192.168.139.10"; then
...
# ---------------------------------------------------------------
{
  echo "Configured by Example Music bindme.sh"
  echo "Hostname    : $(hostname)"
  echo "DNS IP      : ${DNS_IP}"
  echo "Zone        : jukebox.internal"
  echo "Serial      : ${SERIAL}"
  echo "Interface   : ${PROV_IFACE} (${PROV_MAC})"
  echo "Date        : $(date -u +%Y-%m-%dT%H:%M:%SZ)"
} > /etc/.i_am_a_dns_server
chmod 0444 /etc/.i_am_a_dns_server
...
echo -e "${CYAN}    dig @${DNS_IP} -x 192.168.139.10${NC}"
echo -e "${GREEN}============================================================${NC}"
echo
