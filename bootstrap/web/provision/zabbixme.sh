...
#                     on-box edit needed.
# v1.17.0 2026-09-02  Robert re-ran on EXAZABVRK001 with v1.16.0's SSO fix in place -- the initial
#                     Admin/zabbix login now succeeded and "Admin password rotated..." printed, but
#                     Section 13 still showed "No valid API auth token", meaning the re-auth with
#                     the newly-rotated password failed straight after. Found the cause by reading
#                     the code, not guessing: the user.update call that actually performs the
#                     rotation had its response piped straight to /dev/null, with zero check on
#                     whether the API accepted the new password -- if it silently failed (e.g. a
#                     password policy rejection), the script still wrote the *attempted* new
#                     password to /root/.zabbix_api_credentials and printed a success message,
#                     even though the live password on the server was never actually changed. The
#                     subsequent re-auth then correctly failed, because it was authenticating with
#                     a password that was never applied. Fixed: user.update's response is now
#                     captured and checked for a genuine `.result` before the credentials file is
#                     written or the success message is printed; on failure, the real `.error` is
#                     printed, the credentials file is left untouched, and Section 13 proceeds
#                     using the still-valid original Admin/zabbix session token instead of losing
...
EOF
      ...
      if [[ -z "${ZBX_API_AUTH}" ]]; then
        warn "Password rotation appeared to succeed but re-auth with the new password failed."
        warn "API response was: $(jq -c '.error // .' <<< "${ZBX_LOGIN_RESPONSE}" 2>/dev/null || echo "${ZBX_LOGIN_RESPONSE}")"
      fi
    else
      warn "Admin password rotation (user.update) failed -- the live password is still the"
      warn "Zabbix default (Admin/zabbix). Not writing ${API_CREDS_FILE} with an unapplied"
      warn "password. API response was: $(jq -c '.error // .' <<< "${ZBX_UPDATE_RESPONSE}" 2>/dev/null || echo "${ZBX_UPDATE_RESPONSE}")"
      warn "Continuing Section 13 with the still-valid Admin/zabbix session token."
    fi
  else
