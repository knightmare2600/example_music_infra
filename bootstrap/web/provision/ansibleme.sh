...
# Primary inventory locations.
# ansible/inventory/  — per-site files generated by generate_inventory.py (hostnames + groups)
# configs/inventory/  — global flat inventory (main.ini), DC onboarding, Rudder
inventory = ${ANSIBLE_DIR}/inventory,${CONFIGS_DIR}/inventory
...
mapfile -t MANAGED_IPS < <(
  {
    grep -hE '^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+' "${CONFIGS_DIR}/inventory/"*.ini 2>/dev/null \
      | awk '{print $1}'
    grep -hE 'ansible_host=' "${ANSIBLE_DIR}/inventory/"*.ini 2>/dev/null \
...
\${WH}╚══════════════════════════════════════════════════════════════╝\${NC}
  ...
  \${WH}── Ansible ───────────────────────────────────────────────────\${NC}
    ...
    \${CY}Inventory\${NC}: \${GR}${ANSIBLE_DIR}/inventory + ${CONFIGS_DIR}/inventory\${NC}
...
jq -n \
  ...
  --arg inventory      "${ANSIBLE_DIR}/inventory,${CONFIGS_DIR}/inventory" \
...
echo -e "${CYAN}  Inventory     : ${ANSIBLE_DIR}/inventory + ${CONFIGS_DIR}/inventory${NC}"
