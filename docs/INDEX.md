...
| [proxmox/pxe-proxmox-autoinstall-build-log.md](proxmox/pxe-proxmox-autoinstall-build-log.md) | NET-PVE-AUTOINST-001 | Historical build log — abandoned official-ISO/BMC approach, and `select-pve-answer.sh` (built but never tested) — superseded as the current path by NET-PVE-KLARGORING-001 |
| [proxmox/NET-PVE-KLARGORING-001-klargoring-pve-install.md](proxmox/NET-PVE-KLARGORING-001-klargoring-pve-install.md) | NET-PVE-KLARGORING-001 | PVE auto-install via `klargoring` (real PXE) — the current, confirmed-working install path, first proven live on `EXAPVEVRK002` 2026-08-20 |
