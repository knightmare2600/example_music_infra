...
At verbosity 1 and above, the full result dict is printed underneath every **ok/changed**
line — the same detail `-v` always gives you, just indented to stay readable under
`exa_pretty`'s formatting instead of replacing it. No change at all with no `-v` flags.

**Corrected, 2026-09-19** — this used to also gate **failed**/**unreachable** lines behind
`-v`, which meant a failure showed only its terse top-level `msg` (real example:
`non-zero return code`, with the actual `rc`/`stdout`/`stderr`/`cmd` that explain *why*
completely hidden) unless you already knew to re-run with `-v` first. That's backwards —
routine ok/changed output being terse by default is the right call, but the one case that
most needs full detail is exactly the one it was withheld from. **Failed and unreachable
results now always show the full result dict, with no `-v` needed** — verified live: the
same `command: /bin/false` failure that used to print only `non-zero return code` now shows
`cmd`, `rc`, `stdout`, and `stderr` immediately, no flags required. `-v`/`-vv`/`-vvv` still
control ok/changed detail exactly as before.
