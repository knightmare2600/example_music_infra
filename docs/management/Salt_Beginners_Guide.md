...
| 2026-08-19 | Correction to the 2026-08-10 row above: `bootstrap/web/windows/` was restructured into `x86_64/`+`arm64/` briefly, then renamed to `amd64/`+`arm64/` later the same day (per `82-salt-minion.yml`'s own v1.3.1 changelog) — this doc's install-command section still pointed at `{x86_64,arm64}/`. Fixed §3's install path to the real, current `{amd64,arm64}/`. |
...
**Install it** (adjust the filename for your target's actual architecture — note the real committed files live under per-arch subfolders, `bootstrap/web/windows/{amd64,arm64}/`, not flat in `windows/` itself):
