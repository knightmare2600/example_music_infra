...
> **arm64 (2026-08-18):** The script now detects the target node's architecture (`detect_node_arch()`, once per session) and sends `machine=virt` + a real seabios/ovmf choice for arm64 targets — both verified booting live on a real arm64 PVE node before this was written, not assumed. Not yet run through the script's own interactive flow end-to-end against real arm64 hardware, only the underlying `qm` primitives — try `--dry-run` first against a real arm64 node. `convert-v2v.py` still has the equivalent gap, out of scope for this pass.
