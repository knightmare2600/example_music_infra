...
**Auto-heal, added 2026-09-17**: if the connectivity test fails, the play no longer just reports it — it prompts (masked input, never via `-e`/extra-vars) for the target's **root** password, then creates/repairs the `ansible` user and installs whatever public key is *currently* configured, and retries the connectivity test automatically. This matters most for a real, physical PVE hypervisor onboarded without the TOML answer-file path (the documented route seeds the `ansible` user and key automatically; this covers the case where an engineer skipped or didn't know about it) — for bare metal, unlike a VM, "just rebuild it" isn't a realistic recovery option, and Ansible existing specifically to avoid manual per-node SSH cleanup makes this worth having built in rather than done by hand every time. Leave the password prompt blank, or pass `-e ssh_key_preflight_autoheal=false`, to skip auto-heal deliberately and get the original showstopper failure instead.

If auto-heal doesn't apply (e.g. root password login is disabled, or you skipped it) and you hit this:
