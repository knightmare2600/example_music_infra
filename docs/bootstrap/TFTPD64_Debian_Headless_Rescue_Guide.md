...
Designed for completely headless servers with no monitor, no serial console, no keyboard —
network-only access.

> **Note:** the network shown throughout this guide (`192.168.1.0/24`) is a generic lab
> example, not one of Example Music's real site subnets — this is a standalone TFTPD64
> reference for a Windows-hosted rescue/PXE lab, unrelated to the estate's own `benarbejde/`
> site addressing.
...
- Base Directory: `C:\TFTP-Root`
- Enable: TFTP Server
- Place all boot files in `C:\TFTP-Root`
...
Copy both to `C:\TFTP-Root`.

---

## 4. Auto-Start SSH With No Interaction

We boot the installer in network-console mode so SSH starts automatically.

Create `C:\TFTP-Root\pxelinux.cfg\default` with the following content:
...
LABEL rescue
  KERNEL linux
  INITRD initrd.gz
  APPEND auto=true priority=critical anna/choose_modules=network-console \
    network-console/password=RescuePass network-console/password-again=RescuePass \
    netcfg/disable_dhcp=false netcfg/get_hostname=debian-rescue \
    netcfg/get_domain=local console=tty0
```
...
```bash
ipmitool -I lanplus -H 192.168.1.50 -U ADMIN -P password chassis power status
```
...
This configuration exposes DHCP, DNS and SSH. Use only on isolated lab networks or trusted
VLANs. Do not expose to public networks.
...
No monitor. No serial. No keyboard. Network only.
