...
| 2026-09-18 | §2.3's tree updated: `debian/lvm-bios.seed`/`lvm-efi.seed` renamed to `lvm-bios-trixie.seed`/`lvm-efi-trixie.seed`, plus new `lvm-bios-bookworm.seed`/`lvm-efi-bookworm.seed` siblings. Real bug found live (Robert): the shared preseed's `mirror/suite=stable`/`mirror/codename=trixie` were hardcoded, so every "Debian Bookworm" menu.ipxe entry (added 2026-09-17) silently installed Trixie regardless of which kernel/initrd booted — confirmed even with genuinely checksum-verified Bookworm netboot media manually swapped in. Each release now has its own preseed (`mirror/suite=oldstable`/`mirror/codename=bookworm` for the new Bookworm ones); see `menu.ipxe`'s own v2.14 changelog for the full diagnosis. |
...
│                                    preseeds are debian/lvm-{bios,efi}-{trixie,bookworm}.seed
│                                    (2026-09-18: split per-release, see debian/'s own entry below)
...
│   ├── lvm.seed (stale, see above) / lvm-bios-trixie.seed / lvm-efi-trixie.seed /
│   │     lvm-bios-bookworm.seed / lvm-efi-bookworm.seed  ← per-release+arch preseeds
│   │     (2026-09-18: split from one shared lvm-{bios,efi}.seed -- the shared file's
│   │     mirror/suite+codename were hardcoded to Trixie, so Bookworm installs were
│   │     silently installing Trixie regardless of which kernel/initrd booted; each
│   │     release now has its own preseed, see menu.ipxe's own v2.14 changelog)
...
│   ├── x86_64/Salt-Minion-3008.0-Py3-AMD64.msi  ← committed to git 2026-07-20, restructured into
│   │                                        this per-arch subfolder 2026-08-10 (3008.0, official
│   │                                        Salt Project release, checksum-verified). NOT part of
...
│   ├── arm64/Salt-Minion-3008.0-Py3-ARM64.msi   ← added 2026-08-10 -- a CUSTOM build, not an
│   │                                        official release (native Windows arm64 Salt minion
│   │                                        support doesn't exist upstream yet, tracks 3 still-
│   │                                        open PRs -- see buildsheet-salt-minion.md's own
│   │                                        caveat and the root README's "Upstream
│   │                                        Contributions" section). Version-matched to the
│   │                                        x86_64 build above (both 3008.0).
...
Both MSIs (`bootstrap/web/windows/{x86_64,arm64}/Salt-Minion-3008.0-Py3-{AMD64,ARM64}.msi`) are
genuinely committed to this repo, though — see the tree entry above. Unlike most large binaries
...
build's own, different caveat (a custom build, not an official release, though now version-
matched to x86_64 at 3008.0).
