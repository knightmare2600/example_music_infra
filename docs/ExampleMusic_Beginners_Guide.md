...
### 4.2 FRD (Fredericia Havn) — CLD's DR sister site

**`FRD` is not the same as `FRE`.** `FRE` is the real Fredericia office, near the train station — a normal site like any other. `FRD` (Fredericia Havn — "havn" is Danish for harbour/port) is a special infrastructure site code, not a real office, with its own row in `sites.csv`.

**Broadened 2026-08-04 (Robert)**: FRD isn't just VRK's provisioning backup — think of CLD (Edinburgh datacentre) as "cloud site #1" and FRD as its **sister site**, the place the estate falls over to if CLD becomes unreachable. Robert's own framing: if something takes CLD out entirely, the company fails over to Fredericia Havn — `EXAPBXFRD001` (currently hostnamed `EXAPBXCLD002`, see below) standing in for CLD's own PBX is the concrete, already-real example of what that failover looks like in practice, not a hypothetical. `sites.csv` already reflects this at the data level — FRD shares CLD/VRK's `AnsibleRegion=cloud_site` and `Entity=Example Music Limited`, not the normal Danish `dk_site`/`Example Music (Danmark) ApS` classification `FRE` and every other Danish office gets.

The provisioning-network redundancy described below is the *first*, longest-standing piece of that sister-site relationship (VRK's own backup, in-story a second OVH-style vRACK) — real, and still accurate — but not the whole of what FRD is for. In reality the provisioning role itself is a legal fiction — physically the same MacBook running `python3 -m http.server`, mirroring `/debian` from Edinburgh. Only the IP differs. Alongside that MacBook, though, FRD has a real "site kit": a small Intel NUC running Proxmox VE (`EXAPVEFRD001`) and a 48-port switch (`EXASWIFRD001`) — confirmed by Robert 2026-07-30. `menu.ipxe`/`late_command.sh` detect which one to use by checking the booting machine's own network gateway:
