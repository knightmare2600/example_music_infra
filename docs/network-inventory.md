...
| Date | Change |
|------|--------|
| 2026-09-24 | FAL correction: this table's own Infrastructure section only ever showed a single generic `EXABMCFAL001`/`EXAPVEFAL001` row, never the real second node — added `EXARACFAL002`/`EXAPVEFAL002`, and renamed the first row from generic `BMC` to `RAC (Dell iDRAC)` to match the real, confirmed-2026-09-24 hardware (2 real Dell iDRAC9s, not the generic vendor-unknown placeholder). Confirmed 2 real nodes/iDRACs, not 3 — matches this table's own 2026-03-05 changelog entry below ("FAL RAC corrected to `.2`/`.3`") and the network diagram's 2026-07-31 correction; `docs/site-inventory.md` had separately drifted to say 3, now fixed there too. |
...
| `EXARACFAL001` | RAC (Dell iDRAC) | Dell iDRAC9 | `192.168.76.2` | For Proxmox node 1 |
| `EXARACFAL002` | RAC (Dell iDRAC) | Dell iDRAC9 | `192.168.76.3` | For Proxmox node 2 |
| `EXAPVEFAL001` | Proxmox | — | `192.168.76.5` | PVE node 1 |
| `EXAPVEFAL002` | Proxmox | — | `192.168.76.6` | PVE node 2 |
