...
**Verification of this update is still in progress, deliberately.** The rebuilt packages surviving a routine system update (not just a first install) has been proven by removing the safeguard and forcing the older defect back on the box, on purpose, to confirm the automation notices and repairs it — that specific repair has not yet been re-run end to end since the newly-found file-retrieval defect was also fixed in the same session, and the two need confirming together, not treated as separately proven. Until both are confirmed together, live, the pause recorded above stands exactly as written — what has changed is that a specific, credible, already-partially-proven fix now exists where none did when the pause was first decided, not that the pause has been lifted.
