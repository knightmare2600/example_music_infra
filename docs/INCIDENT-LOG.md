...
|---|---|---|
| [INC-2026-09-17-FRD-CONTROL-NODE](#inc-2026-09-17-frd-control-node--building-a-second-control-node-surfaced-three-provisioning-bugs-and-a-genuine-upstream-ansible-defect) | 2026-09-17 | Building a genuinely independent second Ansible control node for FRD immediately proved the value of not relying on a single one — the exercise found and fixed three real provisioning-script bugs, and surfaced an upstream `ansible-core` defect serious enough that FRD's own control node and DNS server rollout are paused pending an upstream fix |
