...
| Hoover script | `/usr/local/bin/pve-monthly-hoover.sh` | Monthly cleanup — journals, temps, coredumps, old Proxmox kernels |
...
| Old Proxmox kernel cleanup | `pve-monthly-hoover.sh` (section 5) | 1st of month |
...
> **Proxmox kernel policy:** The hoover script keeps the **running kernel** and the **latest installed kernel**. All older `proxmox-kernel-*.*.*-*-pve-signed` versioned packages are purged. Meta packages (e.g. `proxmox-kernel-6.17`, `proxmox-kernel-6.8`) and `proxmox-kernel-helper` are **never touched** — the dpkg query pattern `proxmox-kernel-*.*.*-*-pve-signed` excludes them automatically. Both the running and latest versioned packages are preserved because PVE nodes are rarely rebooted — keeping the latest allows a rollback path after the next maintenance window.
...
# How many versioned Proxmox kernels are installed?
...
| Date | Change |
|------|--------|
| 2026-06-09 | v1.2.1 — Fixed `xargs log_info` calls (xargs cannot invoke shell functions); replaced with `log_info "$(…)"` command substitution. Two occurrences in disk-usage reporting lines. |
| 2026-06-09 | v1.2 — PVE 9.x package naming fix throughout: all `pve-kernel-*` references updated to `proxmox-kernel-*.*.*-*-pve-signed`; running kernel detection fixed (`proxmox-kernel-${uname -r}-signed`); stale `pve-headers-*` removal code removed (no separate header packages in PVE 9.x); unattended-upgrades blacklist updated to include `proxmox-kernel.*`; meta packages and `proxmox-kernel-helper` exclusion documented. |
...
| ✅ | Cleans up old **Proxmox kernels** monthly (handled by hoover script — keeps running + latest versioned; never touches meta packages or `proxmox-kernel-helper`) |
...
> **Note on blacklist entries:** `proxmox-kernel.*` covers PVE 9.x package naming; `pve-kernel.*` and `pve-headers.*` are retained as legacy entries in case older-style package names appear. The `pve-manager`, `pve-qemu-kvm`, `pve-container`, `pve-ha-manager`, and `pve-cluster` entries should be verified against your actual installed package names if you have concerns — check with `dpkg -l | grep pve`.
...
**You must not see:** `proxmox-ve`, `proxmox-kernel`, `pve-kernel`, `pve-manager`, or any other Proxmox package.
...
- **Proxmox kernel cleanup** — purges old `proxmox-kernel-*.*.*-*-pve-signed` packages, keeping the running kernel and the latest installed versioned kernel. Meta packages (e.g. `proxmox-kernel-6.17`) and `proxmox-kernel-helper` are excluded by the query pattern and are never touched.
...
[Unit]
Description=Proxmox VE Monthly Hoover - Clean Journals, Temps, Coredumps, Old Proxmox Kernels
...
Timers running:
  ...
  pve-monthly-hoover.timer     — 1st of month, 02:00, disk cleanup + old Proxmox kernel cleanup

Proxmox kernel state at time of setup:
  Running : $(uname -r)
  Installed (versioned signed packages):
$(dpkg -l 'proxmox-kernel-*.*.*-*-pve-signed' | awk '/^ii/ {print "    " $2}' | sort -V)
...
- [ ] Script test run shows correct running and latest kernel in log output
...
<details>
<summary>The hoover log says "0 versioned Proxmox kernels installed — nothing to remove"</summary>

This means the script found no packages matching `proxmox-kernel-*.*.*-*-pve-signed`. Verify what kernel packages are actually installed:
...
If your packages are named `pve-kernel-*` (PVE 8.x style), this procedure document is for PVE 9.x and may not apply to your node. Contact your senior admin.

If packages are `proxmox-kernel-*.*.*-*-pve-signed` and still not being found, re-deploy the script from Step 10 and confirm you are running v1.2.0 or later (`head -10 /usr/local/bin/pve-monthly-hoover.sh`).
...
Confirm both `proxmox-kernel.*` and `pve-manager` appear in `Package-Blacklist`. If the file is missing or truncated, re-run Step 4. Contact your senior admin if the problem persists.
...
<details>
<summary>A Proxmox kernel was removed that should not have been</summary>
...
If the running kernel package (`proxmox-kernel-$(uname -r)-signed`) is missing from dpkg, the system is in an inconsistent state — contact your senior admin immediately. Do not reboot until this is resolved.

If `proxmox-kernel-helper` is missing, reinstall it immediately: `apt-get install proxmox-kernel-helper`. Do not reboot until it is reinstalled.
...
**Why is `proxmox-kernel-helper` never removed?**  
`proxmox-kernel-helper` provides functions used by Proxmox kernel maintenance tasks and must always be present. The hoover script's dpkg query (`proxmox-kernel-*.*.*-*-pve-signed`) does not match it by design — the package name contains no dots and no `-pve-signed` suffix, so it is excluded automatically.

**What are the `proxmox-kernel-6.17` and `proxmox-kernel-6.8` packages? Will they be removed?**  
These are meta packages — they pull in the latest kernel for a given major version and are used during upgrades. The hoover script's query pattern only matches versioned signed packages (e.g. `proxmox-kernel-6.17.4-2-pve-signed`) and will never match meta packages. If a meta package is orphaned after kernel cleanup (e.g. `proxmox-kernel-6.8` after all 6.8 signed packages are removed), `apt-get autoremove` within the script may clean it up — this is expected and safe.
